...
const generateRandomQuestions = async (count = 10) => {
  try {
    ...
    console.log(
      "✅ BULLETPROOF: Selected random questions:",
...
router.post("/random", authenticateToken, async (req, res) => {
  try {
    ...
    let game = await RapidFireGame.findOne({
      ...
      "players.user",
      "username profile.avatar ratings.rapidFireRating"
    );
    ...
    if (game) {
      ...
      console.log(
        "🔍 After joining - game questionSet:",
        game.questionSet.map((id) => id.toString())
      );
      ...
      console.log(
        "🔍 Populated game questionSet after join:",
        game.questionSet.length,
        "questions"
      );
...
router.post("/room", authenticateToken, async (req, res) => {
  try {
    ...
    console.log(
      "✅ Created rapid fire room:",
      game.roomId,
      "with",
      populatedGame.questionSet.length,
      "questions"
    );
...
router.post("/:gameId/finish", authenticateToken, async (req, res) => {
  try {
    ...
    const timeUp =
      new Date() > new Date(game.startTime.getTime() + game.timeLimit * 1000);
    ...
    if (allFinished || timeUp) {
      ...
      if (game.players.length === 2) {
        ...
        if (player1User) {
          ...
          const resultCode =
            player1Result === "win"
              ...
              ? "L"
              : "D";
          player1User.updateRecentGameForm(
            resultCode,
            "rapidfire",
            player1Change
          );
        ...
        if (player2User) {
          ...
          const resultCode =
            player2Result === "win"
              ...
              ? "L"
              : "D";
          player2User.updateRecentGameForm(
            resultCode,
            "rapidfire",
            player2Change
          );
        ...
        console.log("✅ Ratings updated:", {
          player1: {
            old: player1Rating,
            new: player1.ratingAfter,
            change: player1Change,
          },
          player2: {
            old: player2Rating,
            new: player2.ratingAfter,
  ...
  try {
    ...
    if (activePlayers.length === 0 || game.status === "ongoing") {
      ...
      if (winner === "draw") {
        ...
        game.winner = winner;
        game.result = game.players.find((p) => p.status === "left")
          ? "opponent_left"
          : "win";
      }
    }
...
router.get("/leaderboard", async (req, res) => {
  try {
    ...
    const leaderboard = await User.find({
      ...
      .select(
        "username profile.avatar ratings.rapidFireRating rapidFireHistory recentGameForm"
      )
    ...
    const leaderboardWithStats = leaderboard.map((user, index) => {
      ...
      const wins =
        user.rapidFireHistory?.filter((h) => h.result === "win").length || 0;
      const winRate =
      ...
      if (user.recentGameForm && user.recentGameForm.length > 0) {
        const rapidFireGames = user.recentGameForm.filter(
          (game) => game.gameType === "rapidfire"
        );
    ...
    console.log(
      "✅ Retrieved rapid fire leaderboard:",
      leaderboardWithStats.length,
      "users"
    );
