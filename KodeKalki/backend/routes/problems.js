...
router.post("/:id/run", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const judge0Response = await executeCodeWithJudge0(
      code,
      language,
      publicTestCases,
      true
    ...
    console.log(
      "✅ Code execution completed. Passed:",
      passedTests,
      "Total:",
      totalTests
    );
...
router.post("/:id/submit", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await Problem.findByIdAndUpdate(problem._id, {
      ...
      $set: {
        acceptanceRate:
          ((problem.accepted + (isAccepted ? 1 : 0)) /
    ...
    if (isAccepted) {
      try {
        potdResult = await POTDService.awardPOTDCoins(
          req.user._id,
          problem._id
        );
        if (potdResult.awarded) {
          console.log(
            "🪙 POTD coins awarded:",
            potdResult.coinsEarned,
            "Total coins:",
            potdResult.totalCoins
          );
        } else {
  ...
  try {
    ...
    for (let i = 0; i < casesToTest.length; i++) {
      ...
      try {
        ...
        const submissionResponse = await makeJudge0Request(
          "https://judge0-ce.p.rapidapi.com/submissions?base64_encoded=false&wait=true",
          {
            method: "POST",
            headers: {
        ...
        if (!submissionResponse.ok) {
          ...
          throw new Error(
            `Judge0 API error: ${submissionResponse.status} - ${submissionResponse.statusText}`
          );
        }
        ...
        const passed =
          !hasError && actualOutput === expectedOutput && statusId === 3;
        ...
        if (hasError || statusId === 6 || statusId === 5) {
          // Compilation Error or Time Limit Exceeded
          console.log(
        ...
        console.error(
          `❌ Judge0 execution error for test case ${i + 1}:`,
          error.message
        );
        ...
        try {
          const fallbackResult = await executeCodeLocally(
            code,
            language,
            testCase
          );
    ...
    console.log(
      "✅ Judge0 execution completed. Passed:",
      passedTests,
      "Total:",
      testResults.length
    );
  ...
  try {
    ...
    if (lines.length > 1) {
      ...
      if (
        ...
        codeToAnalyze.includes("sorted") ||
        codeToAnalyze.includes("bubble") ||
        codeToAnalyze.includes("selection") ||
        codeToAnalyze.includes("merge") ||
        codeToAnalyze.includes("quick")
      ) {
        console.log("🔍 Detected: Sorting problem");

        // Check if expected output matches sorted array
        const sorted = [...numbers].sort((a, b) => a - b);
        const expectedNums = expectedOutput
          .trim()
          .split(" ")
          .map((n) => parseInt(n));

        if (
          sorted.length === expectedNums.length &&
      ...
      if (
        codeToAnalyze.includes("max") ||
        codeToAnalyze.includes("maximum") ||
      ...
      if (
        codeToAnalyze.includes("min") ||
        codeToAnalyze.includes("minimum") ||
      ...
      // Sum calculation
      if (
        codeToAnalyze.includes("sum") ||
        codeToAnalyze.includes("total") ||
      ...
      if (
        codeToAnalyze.includes("count") ||
        codeToAnalyze.includes("length") ||
...
router.get("/:id/editorial", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching editorial", error: error.message });
  }
});
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: "Error updating editorial", error: error.message });
    }
  }
);
...
router.get("/:id/submissions", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const problemSubmissions = user.submissions
      .filter((sub) => sub.problem.toString() === req.params.id)
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const existingIndex = problem.referenceSolution.findIndex(
      ...
      res.json({
        message: "Solution added/updated successfully",
      ...
      res
        .status(500)
        .json({ message: "Error saving solution", error: error.message });
    }
  }
);
...
router.get("/company/:company", async (req, res) => {
  ...
  try {
    ...
    // Get all problems for stats and paginated problems
    const allProblems = await Problem.find(query);
    
    // ✅ FIX: Deduplicate problems by _id before pagination
    const uniqueProblems = Array.from(
      new Map(allProblems.map(p => [p._id.toString(), p])).values()
    );

    // Apply pagination to deduplicated problems
    ...
    // Populate createdBy for paginated problems only
    await Problem.populate(paginatedProblems, {
      path: 'createdBy',
      select: 'username'
    });

    // Calculate stats from unique problems
    const stats = {
      totalProblems: uniqueProblems.length,
      easyProblems: uniqueProblems.filter((p) => p.difficulty === "Easy").length,
      mediumProblems: uniqueProblems.filter((p) => p.difficulty === "Medium").length,
      hardProblems: uniqueProblems.filter((p) => p.difficulty === "Hard").length,
      avgAcceptanceRate:
        uniqueProblems.reduce((acc, p) => acc + (p.acceptanceRate || 0), 0) /
        uniqueProblems.length || 0,
    };

    console.log("✅ Company data fetched:", {
      problemsCount: paginatedProblems.length,
      uniqueTotal: uniqueProblems.length,
      stats: stats,
    });

    // Remove sensitive fields
    const sanitizedProblems = paginatedProblems.map(p => {
    ...
    res.json({
      ...
      stats,
      totalPages: Math.ceil(uniqueProblems.length / limit),
