...
const Card: React.FC<{ children: React.ReactNode; className?: string; onClick?: () => void }> = ({ children, className = "", onClick }) => (
  <div className={`bg-white rounded-xl shadow-lg border border-gray-100 dark:bg-gray-800 dark:border-gray-700 ${className}`} onClick={onClick}>{children}</div>
)
...
const ContestProblems: React.FC = () => {
  ...
  // Allow navigation even with temp IDs for now
  // We'll handle validation in the problem detail page
  ...
  const isProblemSolved = (problemId: string) => {
    if (!user || !user._id || !contest) return false

    const participant = contest.participants.find((p) => {
      // Safely convert user._id to string, handling undefined case
      const userId = user._id?.toString()
      return userId && p.user._id.toString() === userId
    })
    ...
    const hasSolvedSubmission = participant.submissions.some((sub) => {
      // sub.problem 是字符串类型，直接比较
      return sub.problem.toString() === problemId.toString() && sub.score > 0
    })
  ...
  if (!contest) {
    return (
      <div className={`min-h-screen flex items-center justify-center ${pageBackgroundClass}`}>
        <div className="text-center">
          <div className="text-2xl font-bold text-gray-700 dark:text-gray-300 mb-4">Contest Not Found</div>
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          <div className="font-bold text-lg text-gray-900 dark:text-white">
                            ...
                            {user && participant.user._id === user._id && (
