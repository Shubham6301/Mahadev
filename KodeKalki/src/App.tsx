...
import EditDocumentTab from './components/Admin/EditDocumentTab';
...
const EditDocumentWrapper = () => {
  ...
  return <EditDocumentTab documentId={documentId} onBack={handleBack} />;
};

// ✅ NEW: Wrapper component for viewing a single document
const ViewSingleDocumentWrapper = () => {
  const { documentId } = useParams<{ documentId: string }>();
  const navigate = useNavigate();
  ...
  return (
    <div className={`max-w-4xl mx-auto p-6 ${isDark ? 'text-white' : 'text-gray-900'}`}>
      ...
      <article className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-lg p-8`}>
          ...
          <div className="flex items-center gap-4 mt-4 text-sm text-gray-500 dark:text-gray-400">
            {document.subject && (
              <span className="flex items-center gap-1">
                {document.subject.icon} {document.subject.name}
              </span>
            )}
        ...
        <div className={`prose max-w-none ${isDark ? 'prose-invert' : ''}`}>
          {document.blocks && document.blocks.map((block: any, index: number) => {
            switch (block.type) {
              case 'heading':
              ...
              case 'blockquote':
                return (
                  <blockquote key={index} className="border-l-4 border-gray-300 dark:border-gray-600 pl-4 italic">
                    {block.content}
                  </blockquote>
                );
              default:
        ...
        <div className="pt-0 relative z-10">
          <Routes>
           ...
           {/* Admin Dashboard */}
            ...
            {/* Documents Routes */}
            <Route path="/admin/documents" element={<ProtectedRoute><ViewDocumentsTab /></ProtectedRoute>} />
            <Route path="/admin/documents/add" element={<ProtectedRoute><AddDocumentTab /></ProtectedRoute>} />

            {/* View/Edit single document */}
                  <Route 
                  path="/admin/document/:documentId" 
                 element={<ProtectedRoute><ViewSingleDocumentWrapper /></ProtectedRoute>} 
                 />
                 <Route 
